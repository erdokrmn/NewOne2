...
X_final = np.hstack([X_scaled, y1_arr, x1_arr])  # Y1 ve X1 ölçeklenmez!
...
def append_json_results(new_results, filename="data/m3_results.json"):
    try:
...
append_json_results(results_json, filename="data/m3_results.json")

print(f"\n🔹 En iyi model: {best_name} {best_params} ({best_score:.3f})")
best_model.fit(X_final, y)
with open("data/gaze_grid_m3.pkl", "wb") as f:
    pickle.dump({"model": best_model, "scaler": scaler}, f)
print("✅ Model 3 eğitildi ve data/gaze_grid_m3.pkl olarak kaydedildi.")
