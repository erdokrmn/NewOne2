...
for entry in data:
    ...
    y.append(entry["x3"])  # Model 6 için hedef (İstersen x4 de olabilir)
...
# Model 6 için: scaled features + y1 + x1 + y2 + x2 + y3 
X_final = np.hstack([X_scaled, y1_arr, x1_arr, y2_arr, x2_arr, y3_arr])  # Toplam 24 feature
...
def append_json_results(new_results, filename="data/m6_results.json"):
    try:
...
append_json_results(results_json, filename="data/m6_results.json")

print(f"\n🔹 En iyi model: {best_name} {best_params} ({best_score:.3f})")
best_model.fit(X_final, y)
with open("data/gaze_grid_m6.pkl", "wb") as f:
    pickle.dump({"model": best_model, "scaler": scaler}, f)
print("✅ Model 6 eğitildi ve data/gaze_grid_m6.pkl olarak kaydedildi.")
