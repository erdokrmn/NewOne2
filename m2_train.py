...
# YENİ: Koordinatlı jsonl dosyasını oku (önceki script ile üretmiştin)
...
X_final = np.hstack([X_scaled, y1_arr])  # Y1 ölçeklenmez!

cv = GroupKFold(n_splits=5)

model_configs = {
    "RandomForest": {"class": RandomForestClassifier, "params": {"n_estimators": [500], "max_depth": [20], "n_jobs": [-1], "random_state": [42]}},
    "GradientBoosting": {"class": GradientBoostingClassifier, "params": {"n_estimators": [50], "learning_rate": [0.1], "max_depth": [3], "random_state": [42]}},
...
if XGBClassifier:
    model_configs["XGBoost"] = {"class": XGBClassifier, "params": {"n_estimators": [1000], "max_depth": [8], "learning_rate": [0.01], "eval_metric": ["mlogloss"], "verbosity": [0], "random_state": [42]}}
...
if LGBMClassifier:
    model_configs["LightGBM"] = {"class": LGBMClassifier, "params": {"n_estimators": [1000], "max_depth": [8], "learning_rate": [0.01], "random_state": [42], "verbosity": [0]}}
...
def append_json_results(new_results, filename="data/m2_results.json"):
    try:
...
append_json_results(results_json, filename="data/m2_results.json")
...
print("✅ Model 2 eğitildi ve data/gaze_grid_m2.pkl olarak kaydedildi.")
