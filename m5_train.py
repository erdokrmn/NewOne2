...
for entry in data:
    ...
    y.append(entry["y3"])  # Model 5 için hedef y3 (değiştirerek x3 de yapabilirsin)
...
# Model 5 için: scaled features + y1 + x1 + y2 + x2
X_final = np.hstack([X_scaled, y1_arr, x1_arr, y2_arr, x2_arr])  # Toplam 22 feature
...
def append_json_results(new_results, filename="data/m5_results.json"):
    try:
...
append_json_results(results_json, filename="data/m5_results.json")

print(f"\n🔹 En iyi model: {best_name} {best_params} ({best_score:.3f})")
best_model.fit(X_final, y)
with open("data/gaze_grid_m5.pkl", "wb") as f:
    pickle.dump({"model": best_model, "scaler": scaler}, f)
print("✅ Model 5 eğitildi ve data/gaze_grid_m5.pkl olarak kaydedildi.")
