...
screen_diag = np.sqrt(SCREEN_WIDTH ** 2 + SCREEN_HEIGHT ** 2)

# 4 köşe ve merkez noktası:
reference_points = [
    (0, 0),  # Sol üst
    (SCREEN_WIDTH, 0),  # Sağ üst
    (0, SCREEN_HEIGHT),  # Sol alt
    (SCREEN_WIDTH, SCREEN_HEIGHT),  # Sağ alt
    (screen_center_x, screen_center_y)  # Merkez
]
...
for entry in data:
    ...
    # Her köşe ve merkez için: (iris ile referans noktası arası mesafe) / ekran çapı
    corner_center_ratios = [
    ...
    feature = [
        ...
        # Yeni eklenen 5 feature:
        *corner_center_ratios
    ]
    X.append(feature)
    y.append(entry["y1"])              # <-- Burada target değişkenini değiştirebilirsin
...
def append_json_results(new_results, filename="data/m1_results.json"):
    try:
...
append_json_results(results_json, filename="data/m1_results.json")
...
feature_names = [
    ...
    "distance_cm_norm", "iris_x_norm", "iris_y_norm",
    "dx_norm", "dy_norm", "distance_to_center", "angle_to_center",
    "h_ratio", "v_ratio",
    "ratio_left_top", "ratio_right_top", "ratio_left_bottom", "ratio_right_bottom", "ratio_center"
]
