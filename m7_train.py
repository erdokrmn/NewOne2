...
for entry in data:
    ...
    x2_list.append(entry["x2"])
    y3_list.append(entry["y3"])
    x3_list.append(entry["x3"])
    y.append(entry["y4"])  # Model 7 için hedef x4 (istersen y4 de olabilir)
...
x2_arr = np.array(x2_list).reshape(-1, 1)
y3_arr = np.array(y3_list).reshape(-1, 1)
x3_arr = np.array(x3_list).reshape(-1, 1)
y = np.array(y)
...
# Model 7 için: scaled features + y1 + x1 + y2 + x2 + y3 + x3 + y4
X_final = np.hstack([X_scaled, y1_arr, x1_arr, y2_arr, x2_arr, y3_arr, x3_arr])  # Toplam 25 feature
...
def append_json_results(new_results, filename="data/m7_results.json"):
    try:
...
append_json_results(results_json, filename="data/m7_results.json")

print(f"\n🔹 En iyi model: {best_name} {best_params} ({best_score:.3f})")
best_model.fit(X_final, y)
with open("data/gaze_grid_m7.pkl", "wb") as f:
    pickle.dump({"model": best_model, "scaler": scaler}, f)
print("✅ Model 7 eğitildi ve data/gaze_grid_m7.pkl olarak kaydedildi.")
